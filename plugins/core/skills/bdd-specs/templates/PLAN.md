...
> Required by the `bdd-specs` skill when no doc-driven-development docs
> (`feature-work/<slug>/plan.md`) are present. Put this at `/docs/PLAN.md`.
...
- Test runner: auto-detected by the skill (go.mod → go test + testify, pyproject.toml/.py → pytest)
